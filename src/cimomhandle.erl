...
init([Repository, ProviderRegistration]) ->
    ProvidersForClass = lists:map(
                          fun({{NameSpace, ClassName}, Provider}) ->
                                  {{string:to_lower(NameSpace),
                                    string:to_lower(ClassName)}, Provider}
                          end, ProviderRegistration),
    State = #state{providersforclass = ProvidersForClass, 
